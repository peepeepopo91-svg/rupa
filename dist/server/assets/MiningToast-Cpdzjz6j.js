...
import { E as EXCHANGE_CONSTANTS, M as MINING_CONSTANTS, a as getCommunityState, b as getExchangeRate, c as catchUpUser, s as saveUser, d as saveCommunityState, e as getUser, f as createUser, h as setCurrentUsername, i as buyRig, j as startMining, k as stopMining, r as repairRig, l as sellRig, m as exchangeBCForGems, n as exchangeGemsForBC, o as getCurrentUsername } from "./miningStore-B8AuPTlT.js";
import { c as createSsrRpc } from "./router-BSxSKO2b.js";
import { z } from "zod";
import { c as createServerFn } from "../server.js";
import { s as saveMiningUser, a as serverCatchUp } from "./miningServer-Bfx69aq4.js";
...
function MiningProvider({ children }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      const comm = getCommunityState();
      const elapsed = serverNow() - comm.lastSolvedAt;
      setNext(MINING_CONSTANTS.BLOCK_INTERVAL_MS - elapsed % MINING_CONSTANTS.BLOCK_INTERVAL_MS);
    }, 1e3);
  ...
  useEffect(() => {
    tickRef.current = setInterval(() => {
      ...
      setUser((prev) => {
        if (!prev) return prev;
        const { user: updated, community: updatedComm } = catchUpUser(prev, now);
  ...
  const purchaseRig = useCallback((tierId) => {
    if (!user) return { error: "Not logged in" };
    const result = buyRig(user, tierId);
  ...
  const exchange = useCallback((amount, direction) => {
    ...
    if (direction === "bc-to-gems") {
      ...
      showToast(`Exchanged ${amount} BC → ${result.gemsGained.toFixed(2)} 💎 Gems`, "success");
