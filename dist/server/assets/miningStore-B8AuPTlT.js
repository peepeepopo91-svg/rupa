...
function migrateUser(raw) {
  ...
  if (typeof raw.exchangeUsedToday === "number" && !Number.isInteger(raw.exchangeUsedToday)) {
    raw.exchangeUsedToday = 0;
  }
  if (typeof raw.exchangeUsedToday === "number" && raw.exchangeUsedToday > 100) {
    raw.exchangeUsedToday = 0;
  }
  return raw;
}
...
function createUser(username) {
  ...
  const user = {
    ...
    exchangeUsedToday: 0,
    exchangeResetAt: Date.now() + 24 * 60 * 60 * 1e3
  };
...
function pickBlockWinner(miners, blockNumber) {
  const total = miners.reduce((s, m) => s + m.hashrate, 0);
  if (total === 0 || miners.length === 0) return "nobody";
  const seed = ((blockNumber * 1664525 + 1013904223 & 4294967295) >>> 0) / 4294967295;
  let cumulative = 0;
...
function catchUpUser(user, now, opts) {
  ...
  const elapsed = now - user.lastCheckedAt;
  if (elapsed < 500) return { user, community };
  ...
  const timeSinceLastBlock = now - community.lastSolvedAt;
  const blocksPassed = Math.floor(timeSinceLastBlock / blockIntervalMs);
  ...
  const updatedCommunity = blocksPassed > 0 ? {
    ...community,
    blockNumber: community.blockNumber + blocksPassed,
    lastSolvedAt: community.lastSolvedAt + blocksPassed * blockIntervalMs,
    startedAt: community.lastSolvedAt + blocksPassed * blockIntervalMs,
    totalSolved: community.totalSolved + blocksPassed
  } : community;
  if (!opts?.community && blocksPassed > 0) {
...
function getRateHistory(now = Date.now(), points = 24, intervalMs = 10 * 60 * 1e3) {
...
export {
  ...
  RIG_TIERS as R,
  getCommunityState as a,
  getExchangeRate as b,
  catchUpUser as c,
  saveCommunityState as d,
  getUser as e,
  createUser as f,
  getRateHistory as g,
  setCurrentUsername as h,
  buyRig as i,
  startMining as j,
  stopMining as k,
  sellRig as l,
  exchangeBCForGems as m,
  exchangeGemsForBC as n,
  getCurrentUsername as o,
  getAllUsersForAdmin as p,
  getEconomyOverrides as q,
  repairRig as r,
  saveUser as s,
  saveEconomyOverrides as t
};
