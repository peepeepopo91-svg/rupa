...
// and a Renew button. The countdown is client-local (updates every second) while
// the authoritative expiry time always comes from the server via the User object.
...
export function MiningRenewalBanner() {
  ...
  // Local countdown — display only, does not affect server state
  const [msLeft, setMsLeft] = useState<number>(0)

  // Recalculate time remaining every second from the stored expiry timestamp.
  // Use ?? null to handle undefined on accounts that pre-date this feature.
